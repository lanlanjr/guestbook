...
class User(db.Model, UserMixin):
    ...
    password_hash = db.Column(db.String(128), nullable=True)  # Made nullable to support OAuth users
...
class GuestAccess(db.Model):
    ...
    user = db.relationship('User', backref='accessed_events')
    event = db.relationship('Event', backref='guest_accesses')
    
    def __repr__(self):
        return f'<GuestAccess {self.user_id} to {self.event_id}>' 